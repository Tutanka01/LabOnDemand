...
class TemplateBase(BaseModel):
    ...
    deployment_type: str = Field("custom", pattern=r"^(custom|vscode|jupyter|wordpress)$")
...
class TemplateUpdate(BaseModel):
    ...
    description: Optional[str] = Field(None, max_length=255)
    icon: Optional[str] = Field(None, max_length=100)
    deployment_type: Optional[str] = Field(None, pattern=r"^(custom|vscode|jupyter|wordpress)$")
    default_image: Optional[str] = Field(None, max_length=200)
    default_port: Optional[int] = Field(None, ge=1, le=65535)
